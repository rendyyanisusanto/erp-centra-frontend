<template>
  <div class="report-page">
    <div class="page-header no-print">
      <div>
        <div class="page-title">Laporan Stok Opname Bahan Baku</div>
        <div class="page-subtitle">Laporan Stock Opname Bahan Baku</div>
      </div>
          ...
          <table class="report-table">
            ...
            <tbody>
              <tr v-for="(row, i) in reportData" :key="i">
                ...
                <td class="text-right">{{ fmtStock(row.qty_system) }} {{ row.material.unit }}</td>
                <td class="text-right">{{ fmtStock(row.qty_real) }} {{ row.material.unit }}</td>
                <td>
    ...
    <div v-else-if="reportData && reportData.length === 0" class="card text-center" style="padding: 40px;">
      ...
      <p style="color: #64748b;">No stock opname data available for the selected filters.</p>
    </div>
  </div>
</template>
...
const generateReport = async () => {
  loading.value = true
  try {
    const res = await api.get('/reports/stock-opname', { params: filter.value })
...
const printReport = () => {
  ...
  const url = `/print/stock-opname?${q.toString()}`
...
const exportExcel = async () => {
  ...
  const sheet = workbook.addWorksheet('Stock Opname Report')
  ...
  sheet.getCell('A2').value = 'LAPORAN STOCK OPNAME BAHAN BAKU'
  ...
  headerRow.values = ['No', 'Nama Barang', 'Qty Data', 'Qty Aktual', 'Keterangan']
  ...
  reportData.value.forEach((row, i) => {
    ...
    excelRow.values = [
      i + 1,
      row.material.name,
      `${fmtStock(row.qty_system)} ${row.material.unit}`,
      `${fmtStock(row.qty_real)} ${row.material.unit}`,
      ket
    ]
  ...
  const fileName = `Laporan_Stock_Opname_Bahan_Baku_${new Date().toISOString().split('T')[0]}.xlsx`
