<template>
  <div class="print-page">
    ...
    <div v-if="!loading && reportData && reportData.rows?.length">
        ...
        <div class="report-info-grid">
          <div class="info-row">
            ...
            <span class="info-value fw-600">
              {{ monthName(reportData.month) }} {{ reportData.year }}
            </span>
          </div>
        </div>
    ...
    <div v-if="!loading && (!reportData || !reportData.rows?.length)" class="text-center" style="padding: 40px; color: #64748b;">
...
const monthName = (m) => new Date(2000, Number(m) - 1, 1).toLocaleString('id-ID', { month: 'long' }).toUpperCase()
const fmtQty = (n) => Number(n || 0).toLocaleString('id-ID')

const loadData = async () => {
  try {
    const { month, year } = route.query
    if (!month || !year) throw new Error('Bulan/tahun tidak ditemukan')

    const res = await api.get('/reports/material-issues/monthly', { params: { month, year } })
