  ...
  <div class="report-page">
    ...
    <div class="card" v-if="report">
      <div class="card-header">
        <span class="fw-600">Laporan Piutang</span>
        <span class="fw-700 text-success">Total: {{ fmt(totalSisa) }}</span>
      </div>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-else v-for="r in report" :key="r.id">
              <td>{{ r.id }}</td>
              <td class="fw-600">{{ r.customer }}</td>
              <td>{{ r.date }}</td>
              <td>{{ fmt(r.total_amount) }}</td>
              <td class="text-success">{{ fmt(r.total_paid) }}</td>
              <td class="fw-600 text-success">{{ fmt(r.remaining) }}</td>
...
const fetchReport = async () => {
  loading.value = true
  try {
    const r = await api.get('/reports/receivables', { params: { date_from: dateDari.value, date_to: dateSampai.value } })
...
const printReport = () => {
  ...
  window.open(`/print/receivables?${q.toString()}`, '_blank')
}
...
const exportExcel = async () => {
  ...
  sheet.getCell('A2').value = 'LAPORAN PIUTANG'
  ...
  headerRow.values = ['SO#', 'Pelanggan', 'Tanggal', 'Total', 'Diterima', 'Sisa', 'Status']
  ...
  for (const row of report.value) {
    ...
    r.values = [row.id, row.customer, row.date, Number(row.total_amount), Number(row.total_paid), Number(row.remaining), row.status]
  ...
  saveAs(blob, `Laporan_Piutang_${new Date().toISOString().split('T')[0]}.xlsx`)
}
</script>
