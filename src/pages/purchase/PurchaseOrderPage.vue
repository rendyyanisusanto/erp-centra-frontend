  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="table-wrapper"><table>
        <thead><tr><th>Supplier</th><th>Tanggal</th><th>Total</th><th>Status Dokumen</th><th>Status Bayar</th><th>Aksi</th></tr></thead>
        <tbody>
          ...
          <template v-else-if="items.length===0"><tr><td colspan="6"><div class="empty-state"><div class="empty-state-icon">🛒</div><h3>Tidak ada data pembelian</h3><button class="btn btn-primary" v-if="auth.can('purchase.create')" @click="openCreate">Tambah Pembelian</button></div></td></tr></template>
          <tr v-else v-for="item in items" :key="item.id" @click="openDetail(item)" style="cursor:pointer">
            ...
            <td @click.stop>
              <div class="action-btns">
                ...
                <button class="btn btn-sm btn-warning" v-if="item.status==='DRAFT' && auth.can('purchase.create')" @click="openEdit(item)">Edit</button>
                <button class="btn btn-sm btn-primary" v-if="item.status==='DRAFT' && auth.can('purchase.approve')" @click="approveItem(item)">Setujui</button>
                <button class="btn btn-sm btn-secondary" v-if="item.status==='APPROVED' && auth.can('purchase.approve')" @click="cancelItem(item)">Batalkan</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <BaseModal v-if="showDetail && selectedItem" :title="`PO #${selectedItem.id} - ${selectedItem.supplier?.name}`" size="lg" @close="showDetail=false">
      <div class="detail-info-grid" style="margin-bottom:16px">
        ...
        <div class="detail-info-item"><label>Sisa Hutang</label><p class="fw-700">{{ fmt(selectedItem.remaining_amount) }}</p></div>
      ...
      <template #footer>
        ...
        <button class="btn btn-warning" v-if="selectedItem?.status==='DRAFT' && auth.can('purchase.create')" @click="openEdit(selectedItem)">Edit</button>
        ...
        <button class="btn btn-secondary" v-if="selectedItem?.status==='APPROVED' && auth.can('purchase.approve')" @click="cancelItem(selectedItem)">Batalkan</button>
      </template>
    </BaseModal>
  </div>
</template>
...
const openCreate=()=>{editingId.value=null;form.value={supplier_id:'',date:new Date().toISOString().split('T')[0],description:'',details:[{item_type:'raw_material',item_id:'',qty:1,price:0}]};showForm.value=true}
const openEdit=async(item)=>{
  try{
    ...
    form.value={
      ...
      details:(data.details||[]).map(d=>({item_type:d.item_type||'raw_material',item_id:d.item_id,qty:Number(d.qty||0),price:Number(d.price||0)}))
    }
    if(!form.value.details.length) form.value.details=[{item_type:'raw_material',item_id:'',qty:1,price:0}]
    showForm.value=true
