<template>
  <div>
    <div class="page-header">
      ...
      <button class="btn btn-primary" v-if="auth.can('supplier.create')" @click="openCreate">+ Tambah Supplier</button>
    </div>
    <div class="card">
      ...
      <div class="table-wrapper"><table>
        <thead><tr><th>#</th><th>Nama</th><th>No. HP</th><th>Alamat</th><th v-if="auth.can('supplier.create')">Aksi</th></tr></thead>
        <tbody>
          ...
          <template v-else-if="items.length===0"><tr><td colspan="5"><div class="empty-state"><div class="empty-state-icon">🏭</div><h3>No suppliers found</h3></div></td></tr></template>
          <tr v-else v-for="(item,i) in items" :key="item.id">
            ...
            <td v-if="auth.can('supplier.create')"><div class="action-btns"><button class="btn btn-sm btn-secondary" @click="openEdit(item)">✏️</button><button class="btn btn-sm btn-danger" @click="openDelete(item)">🗑️</button></div></td>
          </tr>
        </tbody>
      </table></div>
    ...
    <BaseModal v-if="showForm" :title="editing?'Edit Supplier':'Tambah Supplier'" @close="showForm=false">
...
import BaseModal from '@/components/BaseModal.vue'; import ConfirmDialog from '@/components/ConfirmDialog.vue'
...
const fetchItems=async()=>{loading.value=true;try{const r=await api.get('/master/suppliers',{params:{page:page.value,limit:15,search:search.value}});items.value=r.data.data.data;total.value=r.data.data.total}catch(e){toast.error('Terjadi kesalahan')}finally{loading.value=false}}
...
const save=async()=>{saving.value=true;try{if(editing.value)await api.put(`/master/suppliers/${editing.value.id}`,form.value);else await api.post('/master/suppliers',form.value);toast.success('Data berhasil disimpan');showForm.value=false;fetchItems()}catch(e){toast.error(e.response?.data?.message||'Terjadi kesalahan')}finally{saving.value=false}}
const doDelete=async()=>{deleting.value=true;try{await api.delete(`/master/suppliers/${deleteTarget.value.id}`);toast.success('Data berhasil dihapus');showDeleteConfirm.value=false;fetchItems()}catch(e){toast.error('Terjadi kesalahan')}finally{deleting.value=false}}
